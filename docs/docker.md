...
```bash
docker build -t awsglue-techadapter .
```
...
```bash
docker run -d --name glue-ta-container -p 8888:8888 awsglue-techadapter
```
...
```bash
docker run -d --name glue-ta-container \
-e OTEL_EXPORTER_OTLP_ENDPOINT=<CollectorURL> \
-e OTEL_METRICS_EXPORTER=otlp \
-e OTEL_SERVICE_NAME=glue-tech-adapter \
-e OTEL_TRACES_EXPORTER=otlp \
-p 8888:8888 awsglue-techadapter
```
...
```bash
docker run -d --name awsglue-ta-container \
